...
AC_DEFUN([MATE_ARG_NLS],[
  AC_ARG_ENABLE([NLS], [NLS], [check])
])

AC_DEFUN([MATE_CHECK_NLS],[
  ...
  then
	...
	then
	  enable_nls=no
          ...
          AC_MSG_ERROR([gettext() is required to build]")
	fi
      ])
    ])
  fi
  ...
  then
    AC_CHECK_HEADERS([libintl.h], [enable_nls=yes],[
      ...
      then
        ...
        AC_MSG_ERROR([libintl.h is required to build]")
      fi
    ])
  fi
...
AC_DEFUN([MATE_RESULT_NLS],[
  MATE_RESULT([NLS], [$enable_nls])
])
